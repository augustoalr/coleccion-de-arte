...
function Catalogo() {
  ...
  const [obraAEliminar, setObraAEliminar] = useState(null); // Para saber quÃ© obra borrar
  ...
  const API_URL = 'http://localhost:4000'; // URL base hardcodeada
  ...
  // useEffect para el "debounce" de la bÃºsqueda (Corregido)
  useEffect(() => {
  ...
  }, [terminoBusqueda, pagina]); // Dependencia 'pagina' aÃ±adida
  ...
  const handleConfirmarEliminar = async (masterPassword) => {
    ...
    try {
        // Primero, verificar la contraseÃ±a maestra
        ...
        if (!verifyResponse.ok) {
            ...
            throw new Error(errorText || 'ContraseÃ±a maestra incorrecta.');
        }
        ...
        await handleActionCompleta(); // Usar la nueva funciÃ³n aquÃ­

        fetchObras(busquedaDebounced, pagina);

    } catch (error) {
        console.error("Error al eliminar:", error.message);
        // AquÃ­ podrÃ­as mostrar el error en el modal si lo deseas
  ...
  return (
    <>
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        <TextField
          fullWidth
          label="Buscar por TÃ­tulo, Autor o NÂ° de Registro"
          variant="outlined"
